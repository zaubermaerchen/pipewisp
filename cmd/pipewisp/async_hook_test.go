...
func TestAsyncIdleHookDoesNotBlockNormalProcessing(t *testing.T) {
	...
	go func() {
		done <- runCLI([]string{"--idle", "5ms", "--on-idle.async", command}, input, &output, &diagnostics)
	}()
	...
	case status := <-done:
		if status != 0 {
			t.Fatalf("runCLI() status = %d, want 0; diagnostics = %q", status, diagnostics.String())
		}
	case <-time.After(500 * time.Millisecond):
		t.Fatalf("runCLI() waited for async hook; elapsed = %s", time.Since(startedAt))
	}
...
func TestAsyncIdleHooksMayOverlap(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("runCLI() did not finish after releasing EOF")
	}
...
func TestAsyncIdleFailureIsDiagnosticOnly(t *testing.T) {
	...
	go func() {
		done <- runCLI([]string{"--idle", "5ms", "--on-idle.async", command}, input, io.Discard, diagnostics)
	}()
	...
	case status := <-done:
		if status != 0 {
			t.Fatalf("runCLI() status = %d, want 0", status)
		}
	case <-time.After(time.Second):
		t.Fatal("runCLI() did not finish")
	}
}
...
func TestAsyncResumeHookRunsForResumedData(t *testing.T) {
	...
	go func() {
		done <- runCLI([]string{"--verbose", "--idle", "5ms", "--on-resume.async", hookOutputCommand("async-resume")}, input, &output, diagnostics)
	}()
...
func TestAsyncIdleHookTimeoutIsDiagnosticOnly(t *testing.T) {
	...
	go func() {
		done <- runCLI([]string{"--idle", "5ms", "--on-idle.async", command, "--hook-timeout", "20ms"}, input, io.Discard, diagnostics)
	}()
...
func TestAsyncHookVerboseRecordsStartAndEnd(t *testing.T) {
	...
	go func() {
		done <- runCLI([]string{"--verbose", "--idle", "5ms", "--on-idle.async", "true"}, input, &output, diagnosticEvents)
	}()
