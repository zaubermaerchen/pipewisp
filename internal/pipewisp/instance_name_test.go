...
func TestRunCLINameParseErrorsUseDefaultPrefix(t *testing.T) {
	...
	if got := Run([]string{"--name", "relay", "--unknown"}, strings.NewReader("input"), &output, &diagnostics); got != 1 {
...
func TestRunCLIUsesNamedRuntimePrefix(t *testing.T) {
	...
	if got := Run([]string{"--name", "relay"}, strings.NewReader("input"), errorWriter{err: errors.New("output unavailable")}, &diagnostics); got != 1 {
		t.Fatalf("Run() status = %d, want 1", got)
	}
...
func TestRunCLIUsesNamedPrefixForReadAndShutdownErrors(t *testing.T) {
	t.Run("read error", func(t *testing.T) {
		...
		var output, diagnostics bytes.Buffer
		if got := Run([]string{"--name=relay"}, &dataAndErrorReader{err: wantErr}, &output, &diagnostics); got != 1 {
			t.Fatalf("Run() status = %d, want 1", got)
		}
	...
	t.Run("shutdown hook failure", func(t *testing.T) {
		var output, diagnostics bytes.Buffer
		if got := Run([]string{"--name=relay", "--on-shutdown", failingHookCommand("shutdown-failed")}, strings.NewReader("input"), &output, &diagnostics); got != 1 {
			t.Fatalf("Run() status = %d, want 1", got)
		}
...
func TestRunCLINameOnlyPreservesStreamAndFastPath(t *testing.T) {
	...
	var output, diagnostics bytes.Buffer
	if got := Run([]string{"--name=relay"}, input, &output, &diagnostics); got != 0 {
...
func TestRunCLIUsesNamedVerbosePrefixWithoutFormattingName(t *testing.T) {
	var output, diagnostics bytes.Buffer
	if got := Run([]string{"--name=%s", "--verbose"}, strings.NewReader("input"), &output, &diagnostics); got != 0 {
		t.Fatalf("Run() status = %d, want 0; diagnostics = %q", got, diagnostics.String())
	}
...
func TestRunCLIForwardsNamedHookOutputWithoutPrefix(t *testing.T) {
	var output, diagnostics bytes.Buffer
	if got := Run([]string{"--name", "relay", "--verbose", "--on-ready", hookOutputAndErrorCommand("hook-out", "hook-err")}, strings.NewReader("input"), &output, &diagnostics); got != 0 {
		t.Fatalf("Run() status = %d, want 0; diagnostics = %q", got, diagnostics.String())
	}
	if got := output.String(); got != "input" {
		t.Fatalf("stdout = %q, want input", got)
...
func TestRunCLINamedHookFailureUsesNamedPrefixWithoutVerbose(t *testing.T) {
	...
	if got := Run([]string{"--name=relay", "--on-ready", command}, strings.NewReader("input"), &output, &diagnostics); got != 1 {
		t.Fatalf("Run() status = %d, want 1", got)
	}
...
func TestRunCLINameDoesNotChangeHookEnvironment(t *testing.T) {
	...
	var output, diagnostics bytes.Buffer
	if got := Run([]string{"--name", "relay", "--on-ready", command}, strings.NewReader("input"), &output, &diagnostics); got != 0 {
		t.Fatalf("Run() status = %d, want 0; diagnostics = %q", got, diagnostics.String())
	}
