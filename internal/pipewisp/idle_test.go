...
func TestParseRejectsInvalidIdleOptions(t *testing.T) {
	tests := []struct {
		...
		{name: "idle alone", args: []string{"--idle=1s"}, want: "--idle requires --verbose, --events-fd, --on-idle, --on-idle.async, --on-resume, or --on-resume.async"},
		{name: "idle with name alone", args: []string{"--name", "relay", "--idle=1s"}, want: "--idle requires --verbose, --events-fd, --on-idle, --on-idle.async, --on-resume, or --on-resume.async"},
